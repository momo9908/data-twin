...
- 对升速包络内的正转速、正变形数据执行**弹性/连续弹塑性拟合**：
  - 自动搜索最优分界转速；
  - 弹性段固定 `δ=a·n²`，塑性段指数 `p>2`；允许尚未出现塑性段；
...
- **模型选择**：沿用 NumPy，不新增依赖。第一段指数固定为2，在 log 空间比较
  纯二次模型和连续分段模型。塑性段需满足 `p-2 > max(1e-8, 3×指数标准误)`、
  BIC 改善至少6，整体及弹性段原始变形空间 `R² >= 0.95`。纯弹性模型也要求
  `R² >= 0.95`。这些是可在新模块顶部查看的数值保护参数，不是材料屈服常数；
  尚未经过真实轮盘试验标定，不能作为安全联锁。有效点少于8个或两种模型均不可靠时显示待判定。
